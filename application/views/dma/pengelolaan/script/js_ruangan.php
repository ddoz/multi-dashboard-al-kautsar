...
$(document).ready(function() {
    $(".btnmodalForm").click(function(e) {
        ...
        $("#form").attr("action","<?=base_url()?>dma/pengelolaangedung/simpanruangan");
    });
...
function edit(data) {
    appId = data.app_id;
    $("#modalForm").modal({ backdrop: 'static', keyboard: false });
    $("#nama_ruangan_input").val(data.nama_ruangan);
    $("#id_input").val(data.id);
    $("#form").attr("action","<?=base_url()?>dma/pengelolaangedung/ubahruangan");
}
   ...
   var t =	$('#datatable').DataTable({
        initComplete: function() {
            ...
            serverSide 		: true,
			ajax:{
				url 		: "<?=base_url()?>dma/pengelolaangedung/datatableRuangan/<?=$gedung->id?>",
				type 		: "POST"
			},
			columns 		:[
                {data: 'nama_ruangan'},
